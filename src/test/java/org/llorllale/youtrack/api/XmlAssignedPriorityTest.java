...
/**
 * Unit tests for {@link XmlAssignedPriority}.
 * @author George Aristy (george.aristy@gmail.com)
 * @since 0.6.0
 */
public class XmlAssignedPriorityTest {
  private static Issue<org.llorllale.youtrack.api.jaxb.Issue> issue;

  @BeforeClass
  public static void setup() throws Exception {
    issue = new MockIssue<>(
        new MockProject(), 
        new XmlStringAsJaxb<>(org.llorllale.youtrack.api.jaxb.Issue.class).apply(ISSUE)
    );
  }
  ...
  public void testIssue() {
    assertThat(
        new XmlAssignedPriority(issue, new MockSession()).issue(),
        is(issue)
    );
  }
  ...
  public void testAsString() {
    assertThat(
        new XmlAssignedPriority(issue, new MockSession()).asString(),
        is(issue.asDto().getField()
            .stream()
